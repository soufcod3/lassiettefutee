...
const PlateCard = ({ name, price, rating, image, veggie }: Props) => {
    return (
        <Card className="p-0 w-fit rounded-lg relative">
            ...
            <CardContent className="p-0 h-18 overflow-hidden rounded-lg rounded-b-none flex justify-center items-center relative">
                <Image src={image} alt="Plate" width={100} height={140} />
                <div className="absolute top-0 right-0">
                    <p className="text-xs text-center bg-white rounded-lg px-1 py-1 m-2 flex items-center gap-1">
                        {rating} <BsFillStarFill className="w-3 h-3 text-yellow-500" />
                    </p>
                </div>
            </CardContent>
            <CardFooter className="flex flex-col justify-center items-center px-0 py-2">
